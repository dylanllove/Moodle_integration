...
export async function flashcards(text: string): Promise<Flashcard[]> {
  const raw = await complete(
    `Create study flashcards from this material. Respond with ONLY a JSON object like {"cards":[{"q":"question","a":"answer"}]} — no prose, no markdown fences.\n\nMATERIAL:\n${clamp(text)}`,
    {
...
function parseJsonArray(raw: string): Flashcard[] {
  ...
  try {
    ...
    // JSON mode can only return an object, so cards may arrive wrapped in one.
    const arr: any[] | null = Array.isArray(parsed) ? parsed : Array.isArray(parsed?.cards) ? parsed.cards : null;
    if (arr) {
      return arr
